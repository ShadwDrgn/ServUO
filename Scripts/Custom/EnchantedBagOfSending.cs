...
{
    public enum EnchantedBagOfSendingHue
    {
        Yellow,
        Blue,
        Red,
    ...
    public class EnchantedBagOfSending : Item, TranslocationItem
    {
        private int m_Charges;
        private int m_Recharges;
        private EnchantedBagOfSendingHue m_EnchantedBagOfSendingHue;
        [Constructable]
        public EnchantedBagOfSending()
            : this(RandomHue())
        {
        }

        [Constructable]
        public EnchantedBagOfSending(EnchantedBagOfSendingHue hue)
            : base(0xE76)
        {
            Name = "An Enchanted Bag of Sending";
            Weight = 2.0;

            EnchantedBagOfSendingHue = hue;

            m_Charges = Utility.RandomMinMax(3, 9);
        }

        public EnchantedBagOfSending(Serial serial)
            : base(serial)
        {
        }
        ...
        public string TranslocationItemName => "enchanted bag of sending";
        [CommandProperty(AccessLevel.GameMaster)]
        public EnchantedBagOfSendingHue EnchantedBagOfSendingHue
        {
            get
            {
                return m_EnchantedBagOfSendingHue;
            }
            set
            {
                m_EnchantedBagOfSendingHue = value;

                switch (value)
                {
                    case EnchantedBagOfSendingHue.Yellow:
                        Hue = 0x8A5;
                        break;
                    case EnchantedBagOfSendingHue.Blue:
                        Hue = 0x8AD;
                        break;
                    case EnchantedBagOfSendingHue.Red:
                        Hue = 0x89B;
                        break;
                    case EnchantedBagOfSendingHue.Green:
                        Hue = 0x08A0;
                        break;
                }
            }
        }
        public static EnchantedBagOfSendingHue RandomHue()
        {
            switch (Utility.Random(4))
            {
                case 0:
                    return EnchantedBagOfSendingHue.Yellow;
                case 1:
                    return EnchantedBagOfSendingHue.Blue;
                case 2:
                    return EnchantedBagOfSendingHue.Red;
                default:
                    return EnchantedBagOfSendingHue.Green;
            }
        }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.WriteEncodedInt((int)m_EnchantedBagOfSendingHue);
        }
                ...
                case 0:
                    {
                        ...
                        m_EnchantedBagOfSendingHue = (EnchantedBagOfSendingHue)reader.ReadEncodedInt();
                        break;
                    }
            }
        }
        ...
        {
            private readonly EnchantedBagOfSending m_Bag;
            public UseBagEntry(EnchantedBagOfSending bag, bool enabled)
                : base(6189)
            {
                m_Bag = bag;
        ...
        {
            private readonly EnchantedBagOfSending m_Bag;
            public SendTarget(EnchantedBagOfSending bag)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                else if (targeted is Item)
                {
                    ...
                    else if (item is EnchantedBagOfSending)
                    {
                        int looted = 0;
                        ...
                        foreach ( Item found in player.GetItemsInRange(3) )
                        {
                            if ( found is Corpse )
                            {
                                Corpse corpse = found as Corpse;
                                if ( isCorpseLootable(player, corpse) )
                                {
                                    corpses.Add(corpse);
                                }
                            }
                            else if ( found != null && found.Movable && found.IsAccessibleTo(player) && !found.Deleted && found.Movable && found is Gold)
                            {
                                items.Add(found as Gold);
                            }
                        }
